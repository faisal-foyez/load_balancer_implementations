...
const servers = [
  { name: 'Server 1', url: 'http://localhost:3001', active: 0, weight: 5 },
  { name: 'Server 2', url: 'http://localhost:3002', active: 0, weight: 2 },
  { name: 'Server 3', url: 'http://localhost:3003', active: 0, weight: 1 },
];
...
function chooseServer() {
  ...
  for (const server of servers) {
    const ratio = server.active / server.weight;
    if (ratio < minRatio) {
      minRatio = ratio;
      selected = server;
    }
